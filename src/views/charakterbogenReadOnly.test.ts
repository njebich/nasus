...
describe('Charakterbogen-Untertabs als reine Ausgabe', () => {
  ...
  it('zeigt Spruchmagie, KI und PSI mit denselben aktuellen TaW wie die Arbeitsansichten', () => {
    ...
    const spruchArbeitswert = container.querySelector(`tr[data-referenz="${spell.referenz}"] .kampf-pool-value`)?.textContent;
    ...
    const kiArbeitswert = container.querySelector('tr[data-referenz="ki_konzentration"] .kampf-pool-value')?.textContent;
    ...
    const psiArbeitswert = container.querySelector(`tr[data-referenz="${psiRoot.referenz}"] .kampf-pool-value`)?.textContent;
