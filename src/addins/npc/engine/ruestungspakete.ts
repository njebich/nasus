...
export type NpcArmorOverrides = Record<string, { verarbeitungSourceRow: number; anpassungSourceRow: number }>;
...
export function inspectNpcArmor(character: CharacterState) {
  ...
  while (minimumRm <= Math.ceil(rh) && computeRbe(rh, kon, staerke, minimumRm) > 0) minimumRm++;
  return {
    rh, rbe, be: Number(evalReferenz('gewichtsbelastung', source)), rm, minimumRm,
...
export function applyNpcArmorPackage(character: CharacterState, packageId: string, kopfschutz = false, overrides: NpcArmorOverrides = {}): CharacterState {
  ...
  if (paket) candidate.ruestungSlots = {};
  ...
  for (const [key, override] of Object.entries(overrides)) {
    ...
    const basis = RUESTUNG_BASIS.find((row) => row.sourceRow === previous.basisSourceRow);
    ...
    if (!basis || !verarbeitung || !anpassung) throw new Error('Die gewählte Rüstungskombination existiert nicht.');
  ...
  candidate.values[RM] = Math.max(previousRm, inspectNpcArmor(candidate).minimumRm);
  candidate.notes += `\nRüstung: ${paket?.name ?? 'Vorlagenrüstung mit geänderter Fertigung/Anpassung'}${kopfschutz && paket ? '; zusätzlicher Lederpanzer am Kopf' : ''}.`
    + ` Rüstungsmanöver ${previousRm} → ${candidate.values[RM]}; Ziel: 0 BE aus Rüstung.`;
  return candidate;
}
