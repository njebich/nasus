...
## 2026-09-20 · 049 · GELÖST — Teildivisionen in „Rüstung verstärken“ werden einzeln aufgerundet

Die im vorherigen Schritt ergänzte Auswertung von `{Magie}/3+{Aura}/3` ist regelgerecht korrigiert: Jede Division wird als eigener Teilwert aufgerundet, bevor die Teilwerte addiert werden. Bei Magie 5 und Aura 4 ergibt sich deshalb `AUFRUNDEN(5/3)+AUFRUNDEN(4/3) = 2+2 = 4`; der angezeigte Verstärkungsfaktor ist nun 4 statt 3. Reine alte Slash-Listen wie `12/1/21` bleiben weiterhin unangetastet. Der Regressionstest verwendet den vollständigen Wirkungstext. Die SPOT-Arbeitsmappe wurde nicht gelesen. Der erzeugte Laufzeitdatensatz enthält kein literales `&#x20;`; die in der Rückmeldung sichtbare Zeichenfolge ist daher nicht Bestandteil dieses Regeltexts.

Geändert sind `src/engine/spruchmagieRw.ts` und `src/engine/spruchmagieRw.test.ts`. Gezielte Prüfung: 1 Testdatei mit 27/27 Tests bestanden. Vollständige Prüfung: 70 Testdateien mit 710/710 Tests bestanden. Produktions-Build einschließlich Waffen-Katalogvalidierung und TypeScript-Prüfung bestanden; die bestehende Warnung zur großen JavaScript-Ausgabedatei bleibt unverändert. Commit und Push nach `origin/main` gehören zu diesem Arbeitsschritt. Bestehende lokale Fremdänderungen und unversionierte Dateien wurden erhalten.
...
Nächster vorgeschlagener Schritt für diese Korrektur: Nach Abschluss der GitHub-Pages-Veröffentlichung „Rüstung verstärken“ mit Magie 5 und Aura 4 erneut in der Runtime prüfen. Erwartet wird Faktor 4. Danach bleibt für die NPC-Arbeit die gewichtete Schulwahl anhand vorhandener Spezies-Eigenschaftsboni der nächste fachliche Schritt. Diese Schritte sind nur vorgeschlagen und noch nicht ausgeführt.
